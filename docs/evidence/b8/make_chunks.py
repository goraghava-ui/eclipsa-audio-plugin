...
save(f"{OUT}/panner-flat.b64", pxml, ptail)
save(f"{OUT}/renderer-flat.b64", export_to(rxml, f"{OUT}/bridge_flat.iamf"), rtail)

# B: tent elevation (kTent = 1), front rim, with the position baked into the
...
bxml = elevation(pxml, 1)
bxml = param(bxml, "X", "0.0")
bxml = param(bxml, "Y", "50.0")
bxml = param(bxml, "Z", "0.0")
assert 'elevation="1"' in bxml, "tent elevation not applied"
assert '<PARAM id="Y" value="50.0"/>' in bxml, "Y not applied"
save(f"{OUT}/panner-tent.b64", bxml, ptail)
save(f"{OUT}/renderer-tent.b64", export_to(rxml, f"{OUT}/bridge_tent.iamf"), rtail)
